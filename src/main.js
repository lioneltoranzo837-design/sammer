...
async function loadCurrentJackpot() {
    ...
    try {
        ...
        const validReceiptPayers = new Map((receipts || [])
...
async function attemptJackpotPayout(amountSats) {
    ...
    try {
        ...
        const payoutReceipt = (receipts || [])
            .sort((left, right) => right.created_at - left.created_at)
            .find((receipt) => isMatchingPayoutReceipt(receipt, signedZapRequest, invoiceData.pr, playerNostrPubkey, winnerZapConfig.providerPubkey, amountSats));
        if (!payoutReceipt) {
